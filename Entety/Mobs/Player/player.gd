...
func _init():
	...
	_independent_movement.movement_stoped.connect(save_to_file)
static func init(
	perent: Node, position: Vector2, direction: float, damage: float, path_to_inv: String
) -> Player:
...
static func init_from_file(file_path: String, parent: Node) -> Player:
	...
	var p = Player.init(
		...
		float(line[2]), float(line[3]), file_path.get_base_dir() + '/' + pF.get_line()
	)
